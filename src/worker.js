...
const FOLLOWER_MIN_BACKOFF_MS = 5000;
const WORKER_MAX_ATTEMPTS = parseIntEnv(envTrim("WORKER_MAX_ATTEMPTS"), 25, 1);
...
console.log(
  `[worker] version=${WORKER_VERSION} DRY_RUN=${DRY_RUN} TZ=${FAZUMI_TZ} window_hours=${WHATSAPP_WINDOW_HOURS} burst_seconds=${BURST_SECONDS} worker_max_attempts=${WORKER_MAX_ATTEMPTS}`
);
...
const LEADER_TERMINAL_OUTCOMES = new Set([
  "summary_sent",
  "paywall",
  "privacy_notice_sent",
  "command_processed",
...
function enforceMinFollowerBackoffIso(nextAttemptAtIso) {
  const minMs = Date.now() + FOLLOWER_MIN_BACKOFF_MS;
  const requestedMs = isoToMsOrNull(nextAttemptAtIso);
  const nextMs = requestedMs == null ? minMs : Math.max(minMs, requestedMs);
...
async function parkFollowerForBurst(rowId, leaderEventId, nextAttemptAtIso, reason = null) {
  ...
  const nextAttempt = enforceMinFollowerBackoffIso(nextAttemptAtIso);
  const leaderTail = leaderId ? tailN(leaderId, 4) : "none";
  const outcome = `burst_non_leader_wait_${leaderTail}`;
  const skip =
    typeof reason === "string" && reason.trim().startsWith("burst_wait_")
      ? reason.trim()
      : `burst_wait_${leaderTail}`;
  const patch = stripProvider({
    status: "pending",
    processed_at: null,
  ...
  console.log("[lifecycle] follower_parked", {
    idLast4: tailN(rowKey, 4),
    leader_id_last4: leaderId ? tailN(leaderId, 4) : null,
    next_attempt_at: nextAttempt,
    reason: skip,
  });
}

async function fetchLeaderLifecycleRow(leaderEventId) {
  const leaderId = String(leaderEventId || "").trim();
...
function isLeaderTerminalState(leaderRow) {
  if (!leaderRow) return false;

  const leaderStatus = String(leaderRow.status || "").toLowerCase();
  const leaderOutcome = String(leaderRow.outcome || "").toLowerCase();
  return (
    safeIso(leaderRow.processed_at) != null ||
    leaderStatus === "processed" ||
    leaderStatus === "error" ||
    LEADER_TERMINAL_OUTCOMES.has(leaderOutcome)
  );
}

async function settleFollowerAgainstLeader(rowId, leaderEventId, waNumber, bucketTsIso, reason = null) {
  const rowKey = String(rowId || "").trim();
  const leaderId = String(leaderEventId || "").trim();
  if (!rowKey) return { action: "noop" };

  // Deterministic follower settlement: if leader is terminal, finalize immediately.
  if (leaderId) {
    const leaderRow = await fetchLeaderLifecycleRow(leaderId);
    if (isLeaderTerminalState(leaderRow)) {
      const marked = await markRowAsBatchedChild(rowKey, leaderId);
      if (marked) {
        console.log("[lifecycle] finalize", {
          row_count: 1,
          total_chars: 0,
          id_last4: tailN(rowKey, 4),
          leader_id_last4: tailN(leaderId, 4),
        });
        return { action: "finalized", leaderRow, rowCount: 1, totalChars: 0 };
      }
    }
  }

  let nextAttempt = enforceMinFollowerBackoffIso(null);
  if (waNumber && bucketTsIso) {
    const idleGate = await isIdleWindowSatisfied(waNumber, bucketTsIso);
    nextAttempt = enforceMinFollowerBackoffIso(idleGate.ok ? idleGate.nextAttemptAt : null);
  }

  const parkReason = reason || `burst_non_leader_wait_${leaderId ? tailN(leaderId, 4) : "none"}`;
  await parkFollowerForBurst(rowKey, leaderId || null, nextAttempt, parkReason);
  return { action: "parked", nextAttempt };
}
...
async function claimBurstLeaderForPhone(phoneE164, bucketTsIso, eventId) {
  ...
  try {
    ...
    if (error) {
      console.log("[lifecycle] leader_election", {
        phone_last4: tailN(phone, 4),
        bucket_ts: bucketTs,
        is_leader: false,
        leader_id_last4: null,
        error: "rpc_error",
      });
    ...
    console.log("[lifecycle] leader_election", {
      ...
      bucket_ts: bucketTs,
      is_leader: isLeader,
      leader_id_last4: leaderEventId ? tailN(leaderEventId, 4) : null,
    });
...
async function finalizeBurstChildrenForLeader(phoneE164, bucketTsIso, leaderEventId, leaderOutcome, leaderStatus) {
  const leaderId = String(leaderEventId || "").trim();
  ...
  for (const row of rows) {
    ...
    if (!childId || childId === leaderId) continue;

    const marked = await markRowAsBatchedChild(childId, leaderId);
    if (!marked) continue;
    finalizedCount++;
    console.log("[lifecycle] follower_finalized", {
      idLast4: tailN(childId, 4),
      leader_id_last4: tailN(leaderId, 4),
      leader_outcome: String(leaderOutcome || ""),
      leader_status: String(leaderStatus || ""),
    });
  }
...
async function dequeueEventJson() {
  ...
  for (const candidate of candidates) {
    ...
    const outcomeLower = String(candidate?.outcome || "").toLowerCase();
    const isFollowerWait = outcomeLower.startsWith("burst_non_leader_wait");

    if (currentAttempts >= WORKER_MAX_ATTEMPTS) {
      if (expectedStatus === "leader") {
        continue;
      }

      let terminalLeaderId = null;
      const phone = normalizePhoneE164(
        candidate?.from_phone || candidate?.wa_number || candidate?.meta?.from || candidate?.meta?.wa_number || null
      );
      const bucketTs = safeIso(candidate?.bucket_ts);

      if (isFollowerWait && phone && bucketTs) {
        const { data: peers, error: peerError } = await supabase
          .from("inbound_events")
          .select("id, status, outcome, processed_at, received_at")
          .eq("provider", "whatsapp")
          .eq("from_phone", phone)
          .eq("bucket_ts", bucketTs)
          .neq("id", String(candidate.id))
          .order("processed_at", { ascending: false, nullsFirst: false })
          .order("received_at", { ascending: true })
          .order("id", { ascending: true })
          .limit(20);

        if (!peerError && Array.isArray(peers)) {
          for (const peer of peers) {
            const peerOutcome = String(peer?.outcome || "").toLowerCase();
            if (peerOutcome === "batched_child" || peerOutcome.startsWith("burst_non_leader_wait")) continue;
            if (isLeaderTerminalState(peer)) {
              terminalLeaderId = String(peer?.id || "").trim() || null;
              if (terminalLeaderId) break;
            }
          }
        }
      }
      ...
      if (terminalLeaderId) {
        const marked = await markRowAsBatchedChild(candidate.id, terminalLeaderId);
        if (marked) {
          console.log("[lifecycle] finalize", {
            row_count: 1,
            total_chars: 0,
            id_last4: tailN(candidate.id, 4),
            leader_id_last4: tailN(terminalLeaderId, 4),
          });
          continue;
        }
      }
      ...
      const { error: terminalError } = await supabase
        ...
        .update(stripProvider({
          ...
          skip_reason: `max_attempts_exceeded_${WORKER_MAX_ATTEMPTS}`,
      ...
      if (terminalError) {
        console.log("[lifecycle] max_attempts_terminal_failed", {
          id_last4: tailN(candidate.id, 4),
          phone_last4: tailN(phone || "", 4) || "n/a",
          bucket_ts: bucketTs,
        });
      }
      continue;
    ...
    const shouldBumpAttempts = expectedStatus === "leader" || !isFollowerWait;
    const { data: claimed, error: claimError } = await supabase
      .from("inbound_events")
      .update(stripProvider({
        status: expectedStatus === "leader" ? "leader" : "queued",
        attempts: shouldBumpAttempts ? currentAttempts + 1 : currentAttempts,
        processed_at: null,
      }))
      .eq("id", candidate.id)
      .eq("status", expectedStatus)
    ...
    if (claimed && claimed.id != null) {
      console.log("[lifecycle] dequeue", {
        id_last4: tailN(claimed.id, 4),
        phone_last4: tailN(claimed.from_phone || claimed.wa_number || claimed.meta?.from || "", 4) || "n/a",
        bucket_ts: safeIso(claimed.bucket_ts),
        status: String(claimed.status || expectedStatus || "queued"),
      });
      return claimed;
    }
  }
  ...
  while (!shouldStop) {
    ...
    try {
      ...
      if (needsLeaderGate) {
        ...
        if (!leaderClaim.isLeader || String(id) !== leaderEventId) {
