...
## D041 - Shared public/auth pages fill the root footer layout column instead of claiming their own viewport height
**Date:** 2026-03-13
**Context:** FAZUMI's marketing footer is rendered once from `app/layout.tsx`, but several short public/auth wrappers were still using `min-h-screen` or `100dvh` on their own route roots. That made the page content consume the full viewport before the footer was added, so the footer dropped below the fold even on pages where it was supposed to be part of the visible page. The desktop dashboard shell also uses nested scroll panes that should keep scrolling without showing noisy native scrollbar chrome.
